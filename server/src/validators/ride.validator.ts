...
export const createRideSchema = Joi.object({
  departureLocation: Joi.string().trim().max(255).required().messages({
    ...
    "string.base":
      "Le lieu de départ doit être une chaîne de caractères non vide.",
    "string.empty":
      "Le lieu de départ doit être une chaîne de caractères non vide.",
  ...
  arrivalLocation: Joi.string().trim().max(255).required().messages({
    ...
    "string.base":
      "Le lieu d'arrivée doit être une chaîne de caractères non vide.",
    "string.empty":
      "Le lieu d'arrivée doit être une chaîne de caractères non vide.",
  ...
  offeredSeats: Joi.number()
    ...
    .messages({
      ...
      "number.integer":
        "Le nombre de places proposées doit être un nombre entier.",
      "number.min":
