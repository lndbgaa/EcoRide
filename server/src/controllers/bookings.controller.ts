 ...
 */
export const completeBooking = catchAsync(async (req: Request, res: Response): Promise<Response> => {
  const user = req.user;
  const bookingId = req.params.id!;
 ...
 */
export const reportBookingIncident = catchAsync(async (req: Request, res: Response): Promise<Response> => {
  const user = req.user;
  const bookingId = req.params.id!;
  const data = req.body;

  const { booking, incident } = await BookingService.reportIncident(user, bookingId, data);
  const dto = { booking: booking.toPassengerDTO(req.t), incident: incident.toPreviewDTO() };
