 ...
 */
export const getEmployeeAssignedIncidents = catchAsync(
  async (req: Request, res: Response) => {
    ...
    res.status(200).json({
      ...
      data: {
        incidents: incidents.map((incident) => incident.toPreviewDTO()),
      },
      pagination: {
        page,
        limit,
 ...
 */
export const getEmployeeResolvedIncidents = catchAsync(
  async (req: Request, res: Response) => {
    const employeeId = req.user.id;
    const { page, limit, offset } = parsePagination(req);
