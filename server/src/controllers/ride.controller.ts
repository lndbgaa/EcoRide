...
export const searchForRides = catchAsync(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const { count, rides } = await RideService.searchForRides(
      data,
      limit,
      offset,
      userId
...
export const getRideDetails = catchAsync(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const { ride, passengers, preferences } = await RideService.getRideDetails(
      rideId
    );
    ...
    res.status(200).json({
      ...
      data: {
        isDriver,
        ride: isDriver
          ? ride.toDetailedPrivateDTO()
          : ride.toDetailedPublicDTO(),
...
export const getMyRides = catchAsync(
  async (req: Request, res: Response): Promise<void> => {
    ...
    const { count, rides } = await RideService.getUserRides(
      userId,
      limit,
      offset
    );
