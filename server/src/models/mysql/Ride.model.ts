...
export default class Ride extends Model {
  ...
  private transitionTo(newStatus: RideStatus): void {
    ...
    if (!this.canTransitionTo(newStatus)) {
      throw new AppError({
        statusCode: 400,
        userMessage: ERROR_MESSAGES.RIDE.INVALID_STATUS_TRANSITION,
  ...
  public async addAvailableSeats(amount: number, options?: SaveOptions): Promise<void> {
    ...
    if (!canAddSeats) {
      throw new AppError({
        statusCode: 400,
        userMessage: ERROR_MESSAGES.RIDE.CANNOT_MODIFY_SEATS,
    ...
    if (!Number.isInteger(amount) || amount <= 0) {
      throw new AppError({
        statusCode: 400,
        userMessage: ERROR_MESSAGES.RIDE.INVALID_SEAT_AMOUNT,
    ...
    return sequelize.transaction(async (t) => {
      ...
      if (this.available_seats + amount > this.offered_seats) {
        throw new AppError({
          statusCode: 400,
          userMessage: ERROR_MESSAGES.RIDE.EXCEEDS_OFFERED_SEATS,
  ...
  public async removeAvailableSeats(amount: number, options?: SaveOptions): Promise<void> {
    ...
    return sequelize.transaction(async (t) => {
      ...
      if (amount > this.available_seats) {
        throw new AppError({
          statusCode: 400,
          userMessage: ERROR_MESSAGES.RIDE.NOT_ENOUGH_AVAILABLE_SEATS,
