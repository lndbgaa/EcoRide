...
import { Booking, User } from "@/models/mysql";
...
export class BookingService {
   ...
   *
   * @param {User} user - The user instance making the booking.
  ...
  public static async create(user: User, data: CreateBookingPayload): Promise<Booking> {
    ...
    return await sequelize.transaction(async (t) => {
      ...
      const trip = await TripService.findById(tripId, { lock: true, transaction: t });
      ...
      await trip.removeAvailableSeats(seatsToBook, { transaction: t });
   ...
   */
  public static async cancel(userId: string, bookingId: string): Promise<Booking> {
    return await sequelize.transaction(async (t) => {
      const booking = await this.findOwnedById(userId, bookingId, {
      ...
      if (!trip || !passenger) {
        throw new AppError({
          ...
          debugMessage: `[BookingService.cancel] Missing relations for booking '${bookingId}'.`,
        });
      }
